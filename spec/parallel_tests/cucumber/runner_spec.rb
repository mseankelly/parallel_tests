...
describe ParallelTests::Cucumber do
  ...
  describe :run_tests do
    ...
    it "uses TEST_ENV_NUMBER=blank when called for process 0" do
      ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| w['TEST_ENV_NUMBER']==''}.and_return mocked_spawned_process
      call(['xxx'],0,{})
    end

    it "uses TEST_ENV_NUMBER=2 when called for process 1" do
      ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| w['TEST_ENV_NUMBER']=='2'}.and_return mocked_spawned_process
      call(['xxx'],1,{})
    end
    ...
    it "runs bundle exec cucumber when on bundler 0.9" do
      ...
      ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x =~ %r{bundle exec cucumber}}.and_return mocked_spawned_process
      call(['xxx'],1,{})
    end

    it "runs script/cucumber when script/cucumber is found" do
      ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x =~ %r{script/cucumber}}.and_return mocked_spawned_process
      call(['xxx'],1,{})
    end

    it "runs cucumber by default" do
      File.stub!(:file?).with('script/cucumber').and_return false
      ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x !~ %r{(script/cucumber)|(bundle exec cucumber)}}.and_return mocked_spawned_process
      call(['xxx'],1,{})
    end

    it "uses options passed in" do
      ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x =~ %r{script/cucumber .* -p default}}.and_return mocked_spawned_process
    ...
    context "with parallel profile in config/cucumber.yml" do
      ...
      it "uses parallel profile" do
        ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x =~ %r{script/cucumber .* foo bar --profile parallel xxx}}.and_return mocked_spawned_process
        call(['xxx'],1, :test_options => 'foo bar')
      end

      it "uses given profile via --profile" do
        ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x =~ %r{script/cucumber .* --profile foo xxx$}}.and_return mocked_spawned_process
        call(['xxx'],1, :test_options => '--profile foo')
      end

      it "uses given profile via -p" do
        ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x =~ %r{script/cucumber .* -p foo xxx$}}.and_return mocked_spawned_process
    ...
    it "does not use parallel profile if config/cucumber.yml does not contain it" do
      ...
      ParallelTests::Cucumber::Runner.should_receive(:spawn).with{|w,x,y,z| x =~ %r{script/cucumber .* foo bar}}.and_return mocked_spawned_process
